...
function Harness({ onState }: { onState: (left: PanelResult, right: PanelResult) => void }) {
  const left = useFilePanel({ cwd: null, toolState: null, projectRoot: '/tmp/project' })
  const right = useFilePanel({ cwd: null, toolState: null, projectRoot: '/tmp/project' })
  onState(left, right)
  return null
}
...
test('useFilePanel instances are independent', () => {
  ...
  act(() => {
    renderer = create(React.createElement(Harness, { onState: (l, r) => { left = l; right = r } }))
  })
