# x-cli llama-cpp-rs fork instructions

This repository is x-cli's Rust compatibility layer over two upstreams:
...
The containing x-cli repository consumes an exact commit of this repository and records both
...
upstream when x-cli needs capabilities that upstream does not provide. Approved binding work
does not need to wait for an upstream refresh, and useful x-cli functionality must not be
...
1. **Incremental development:** add, change, and fix narrowly scoped bindings on the current
   x-cli generation as product work requires.
2. **Clean-slate refresh:** occasionally analyze the complete x-cli divergence against the
   latest wrapper upstream, start a new generation from that upstream, adopt upstream replacements,
   reimplement only the adaptations that remain necessary, and update x-cli consumers as
   needed.

Incremental work may add whatever binding functionality x-cli requires, subject to the
...
- If latest upstream now provides the required functionality, use its implementation and update
  x-cli for any API differences.
- If x-cli still needs functionality that upstream does not provide, reimplement the necessary
...
Periodic upstream refreshes are clean-slate re-derivations, not merges of the previous x-cli
delta. The goal is not zero divergence; it is a deliberately reviewed, minimal, current divergence
that provides all required x-cli behavior. For a refresh:
...
1. Fetch the latest wrapper upstream. Before changing code, carefully analyze the current fork's
   ...
   describe what x-cli requires rather than assuming each old line must survive:
   ...
   For each substantial divergence, identify its x-cli consumer and determine whether latest
...
3. Do **not** merge, rebase, or cherry-pick the old x-cli bindings branch into the refresh
...
4. First build and test the unmodified wrapper at the llama.cpp revision selected by upstream. This
   separates an upstream baseline failure from a x-cli adaptation failure.

5. If x-cli needs a different llama.cpp revision, select an explicit commit. Never use a
...
6. Reconcile the capability inventory against latest upstream, then point the containing x-cli
   ...
   - If upstream now provides it, change x-cli to use the upstream API.
...
8. Update x-cli consumers as necessary when current upstream offers a better API or the minimal
   refreshed binding differs from the previous generation. Preserve x-cli's documented behavior
...
9. Audit the complete remaining fork delta:
   ...
   Every material addition must have a concrete answer to: "Which x-cli consumer, invariant,
   build target, or parity test fails if this is removed?" Remove additions without such evidence.

10. Publish the refresh commit to `magnitudedev/llama-cpp-rs`, then update the containing x-cli
   repository's nested pointer and both revisions in `inference/native-pin.toml`. Never point
   x-cli at an unpushed bindings commit.

Historical x-cli pins must remain fetchable. Do not force-push away a commit referenced by a
released or shared x-cli revision. A refresh branch or immutable tag may preserve each accepted
generation; the exact commit recorded by x-cli is authoritative.
...
From the containing x-cli repository, run at least:
...
Approved functionality, fixes, and adaptations may be developed normally on the current x-cli
...
clear about why it belongs in the binding instead of x-cli or upstream llama.cpp.
...
decide whether they are adopted from upstream, rewritten minimally, moved into x-cli, or
removed.

## Review rules

- Prefer deletion when upstream has acquired equivalent behavior.
- Do not retain both an upstream implementation and a x-cli compatibility implementation.
