...
// Serialize logger mutations made through this module. Upstream common/fit independently replaces
// the same process-global callback with call-local state, so callers must not run fit concurrently
// with other native work that can log.
static NATIVE_LOGGER_MUTATION: Mutex<()> = Mutex::new(());

pub(crate) fn lock_native_logger() -> MutexGuard<'static, ()> {
    NATIVE_LOGGER_MUTATION
        .lock()
