...
import { measureBounded, renderXmlBodyValue, renderXmlBodyValues, truncate } from '../truncation'
...
describe('XML body truncation', () => {
  test('renders top-level strings as raw XML body text', () => {
    expect(renderXmlBodyValue('packages/agent', 100)).toBe('packages/agent')
  })

  test('keeps JSON-ish formatting for structured values', () => {
    expect(renderXmlBodyValue({ stdout: 'ok' }, 100)).toBe('{stdout: "ok"}')
  })

  test('distributes budget across multiple XML body values', () => {
    const rendered = renderXmlBodyValues(['kept', 'x'.repeat(1000)], 20)
    expect(rendered[0]).toBe('kept')
