...
export type XCliAuthentication =
  | {
...
export function createXCliCatalog(config: MagnitudeCatalogConfig): ModelCatalog<XCliModelInfo> {
  ...
  const fetchModels: Effect.Effect<readonly XCliModelInfo[], ModelCatalogError, HttpClient.HttpClient> =
    Effect.gen(function* () {
      if (authentication._tag === "NotConfigured") {
        return yield* new ModelCatalogError({ message: "x-cli authentication is not configured" })
      }
      ...
      const rawModels = yield* Schema.decodeUnknown(XCliModelListResponseSchema)(body).pipe(
        Effect.mapError((cause) => new ModelCatalogError({ message: "Invalid models response", cause })),
      )

      const classified: XCliModelInfo[] = []
      for (const raw of rawModels.data) {
        const model = toXCliModelInfo(raw)
  ...
  const list: ModelCatalog<XCliModelInfo>["list"] = Effect.gen(function* () {
  ...
  const get: ModelCatalog<XCliModelInfo>["get"] = (_providerId, providerModelId) =>
    Effect.gen(function* () {
  ...
  const refresh: ModelCatalog<XCliModelInfo>["refresh"] = Effect.gen(function* () {
    const models = yield* fetchModels
    cache = models
    fetchedAt = Date.now()
    return models
