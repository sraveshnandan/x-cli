...
const ERROR_TYPES: readonly XCliErrorType[] = [
...
const ERROR_CODES: readonly XCliErrorCode[] = [
  "invalid_api_key",
  "invalid_body",
  "unsupported_field",
  "unsupported_n",
...
function isErrorType(value: unknown): value is XCliErrorType {
  return typeof value === "string" && ERROR_TYPES.includes(value as XCliErrorType)
}

function isErrorCode(value: unknown): value is XCliErrorCode {
  return typeof value === "string" && ERROR_CODES.includes(value as XCliErrorCode)
}
...
export function tryParseErrorBody(body: string): ParsedXCliApiError | null {
  let parsed: unknown
  try {
...
export function classifyXCliRejectedResponse(
  call: ProviderCall,
  response: RejectedHttpResponse,
