...
describe("Magnitude provider authentication", () => {
  it("represents missing authentication through each operation's typed failure channel", async () => {
    ...
    expect(catalog).toMatchObject({ _tag: "Left", left: { _tag: "ModelCatalogError", message: "x-cli authentication is not configured" } })
    expect(webSearch).toMatchObject({ _tag: "Left", left: { _tag: "WebSearchNotConfigured" } })
    expect(usage).toMatchObject({ _tag: "Left", left: { _tag: "XCliClientError", message: "x-cli authentication is not configured" } })
  })

  it("does not relabel a configured auth applicator defect as missing authentication", async () => {
    const instance = createXCliProvider({
      auth: () => {
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        const search = yield* Effect.fork(
          Effect.either(
            createXCliProvider({ apiKey: "cloud_test" }).provider.webSearch("x-cli"),
          ),
        )
